...
public class ShopSwingView extends JFrame implements ShopView {
	...
	public ShopSwingView() {
		...
		listProducts.addListSelectionListener(e -> btnBuySelected.setEnabled(listProducts.getSelectedIndex() != -1));
		...
		btnBuySelected.addActionListener(e -> shopController.buyProduct(listProducts.getSelectedValue()));
		...
		listCart.addListSelectionListener(e -> btnRemoveSelected.setEnabled(listCart.getSelectedIndex() != -1));
		listCart.addListSelectionListener(e -> btnCheckout.setEnabled(listCart.getSelectedIndex() != -1));
		scrollPane_1.setViewportView(listCart);
		listCart.setName("cartList");

		btnRemoveSelected = new JButton("Remove Selected");
		btnRemoveSelected.setEnabled(false);
		btnRemoveSelected.addActionListener(e -> shopController.removeProduct(listCart.getSelectedValue()));
		...
		btnCheckout.addActionListener(
				e -> new Thread(() -> shopController.checkoutProduct(listCart.getSelectedValue())).start());
	...
	public void showAllProducts(List<Product> products) {
		SwingUtilities.invokeLater(() -> products.stream().forEach(listProductsModel::addElement));
	}
	...
	public void showError(String message, Product product) {
		SwingUtilities.invokeLater(() -> lblErrorMessage.setText(message + ": " + product.getName()));
	}
